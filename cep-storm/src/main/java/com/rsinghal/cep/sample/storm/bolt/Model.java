...
public class Model {
	...
	public void train() {
		...
		trainDir = new File("trainDirectory1"); //Directory of training dataset
		categories = trainDir.list();
		int nGram = 7; // the nGram level, any value between 7 and 12 works
		String[] mCategories = { "cards", "channels","general_banking","interest_rate","mortgage","move_money","products","rewards","view_money" }; //categories of the text i.e negative and positive text
		...
		try {
			AbstractExternalizable.compileTo((Compilable) lmc, new File(
					"C:\\Users\\sinravb\\git\\cep-twitter-kafka-storm\\cep-storm\\src\\main\\resources\\classifier_finance.txt"));//saving serialize object to text file
